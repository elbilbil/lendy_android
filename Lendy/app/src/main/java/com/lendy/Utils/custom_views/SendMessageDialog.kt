...
class SendMessageDialog(activity1: Activity, user1: Users?, member1: Member?) : AlertDialog.Builder(activity1) {
        ...
        validate.setOnClickListener { v ->
            ...
            if (message.text.isNotEmpty()) {
                if (user != null) {
                    ...
                    DataManager.sendMessage(context, DataManager.SharedData.token, message.text.toString(), member?._id, callback = { success: Boolean ->
                        if (success) {
                            this.alertDialog!!.dismiss()
                            activity!!.runOnUiThread {
                                android.app.AlertDialog.Builder(activity)
                                        .setTitle("Message envoyé avec succès")
                                        .setMessage("Votre message à bien été envoyé à " + member!!.firstname + " " + member!!.lastname)
                                        .setPositiveButton("OK", null)
                                        .show()
                            }
                        } else {
